...
import { createInscricao, getInscricao, patchProgresso, completeModule } from '../services/progressService.js';
...
export async function completeModuleHandler(req:Request,res:Response,next:NextFunction){
	try { const r = await completeModule(req.params.id, req.params.moduloId); res.status(201).json(r);} catch(e){ next(e);} }
