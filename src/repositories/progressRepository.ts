...
interface InsertInscricao { id:string; funcionario_id:string; curso_id:string; }
export async function insertInscricao(d:InsertInscricao){ await withClient(c=>c.query('insert into progress_service.inscricoes (id, funcionario_id, curso_id, status, progresso_percentual, data_inicio) values ($1,$2,$3,$4,0,now())', [d.id,d.funcionario_id,d.curso_id,'EM_ANDAMENTO'])); }
...
export async function completeModuleDb(inscricaoId:string, moduloId:string){
	return withClient(async c => {
		// recupera inscrição
		const ins = await c.query('select id, curso_id, status, progresso_percentual from progress_service.inscricoes where id=$1',[inscricaoId]);
		...
		return { inscricao_id: inscricaoId, modulo_id: moduloId, progresso_percentual: progresso, concluido: progresso===100, data_conclusao: dataConclusao };
	});
}
